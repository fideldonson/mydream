...
class _MainScreenState extends State<MainScreen> {
  ...
  Widget roedkaelk = Image(image:AssetImage('assets/rodkaelk1.png'), );
  int hedgehogFrame = 0;
  Timer hedgehogTimer;
  Widget hedgehog = Image(image:AssetImage('assets/hedgehog1.png'), );
  int hareFrame = 0;
  Timer hareTimer;
  Widget hare = Image(image:AssetImage('assets/hare1.png'), );
  Widget playIcon = Icon(Icons.play_arrow);

  void showInfo() {
    Navigator.pushNamed( context, 'infoScreen' );
    /* if(Provider.of<MyDreamModel>(context, listen: false).isPending){
      Provider.of<MyDreamModel>(context, listen: false).isPending=false;
    }else{
      Provider.of<MyDreamModel>(context, listen: false).isPending=true;
      
      Future.delayed(Duration(milliseconds: 5000), (){ Provider.of<MyDreamModel>(context, listen: false).isPending=false; });
    } */
  }

  void showInfoChild() {
    // Provider.of<MyDreamModel>(context, listen: false).testConsume();
  ...
  Future playDream(String storyNameParam) async{
    Navigator.push( 
      context,
      MaterialPageRoute(
        builder: (context) => StoryScreen(storyName: storyNameParam,),
      ),
    );
  }
                  ...
                  children: <Widget>[
                    Positioned(
                      ...
                      child:CircleAvatar(
                        ...
                        child: Image(image:AssetImage('assets/Poteknap.png'),fit: BoxFit.none, alignment: Alignment.bottomRight, )
                      ),
                    ),
                    Container(
                      child: AnimatedSwitcher(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: <Widget>[
              ...
              Text('Udregn '+num1.toString()+' + '+num2.toString()+' for at fortsætte', style: TextStyle(fontSize: 20, color: Colors.blueAccent),),
              Container(height: 5,),
  ...
  /* Widget getPendingDialog(){
    if(Provider.of<MyDreamModel>(context, listen: false).isPending){
          ...
          child: Card(
            child: Column(
              ...
              children: <Widget>[
                ListTile(
                  leading: Image(image:AssetImage('assets/modalcorner.png')),
                  title: Text( "Venter på betalingen" ),
                  subtitle: Text( "" ),
                ),
              ],
            ),
      ...
      return Text("not pending");
    }
  }

  void showPendingModal(BuildContext context) {
    ...
    showDialog(
      ...
      context: context,
      builder: (BuildContext context) => pendingDialog
    );
  } */
