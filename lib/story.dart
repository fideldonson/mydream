...
class _StoryScreenState extends State<StoryScreen> {
  ...
  int bearFrame = 0;
  Timer bearTimer;
  Widget sleepingBear = Image(
    image: AssetImage('assets/Sovendebjoern.png'),
  );
  ...
  Future playStory() async {
    ...
    if (storyName == "mydream.bird") {
      ...
      await advancedPlayer.play(AssetSource('coolkids2mix03.mp3'));
    } else if (storyName == "mydream.hare") {
      total = new Duration(minutes: 11, seconds: 22);
      await advancedPlayer.play(AssetSource('bearpractice.mp3'));
    } else if (storyName == "mydream.hedgehog") {
      total = new Duration(minutes: 12, seconds: 26);
      await advancedPlayer.play(AssetSource('beartravel.mp3'));
    } else if (storyName == "mydream.godnat") {
      total = new Duration(minutes: 12, seconds: 26);
      await advancedPlayer.play(AssetSource('beartravel.mp3'));
    }

    advancedPlayer.onPositionChanged.listen(audioProgress);
